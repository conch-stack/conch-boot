...
public class SqlExecutor {
     ...
     */
    public List<Map<String, Object>> executeQuery(String sqlStatement, List params) throws Exception {
     ...
     * 执行更新操作
     */
    public boolean executeUpdate(String sqlStatement, List params) throws Exception {
        if (V.isEmpty(sqlStatement)) {
            return false;
        }
        JdbcTemplate jdbcTemplate = getJdbcTemplate();
    ...
    public static boolean executeUpdate(Connection conn, String sql, List params) throws Exception {
        ...
        if (V.notEmpty(params)) {
            log.debug("==>  Params: {}", GsonUtils.toJson(params));
            if (params.size() > 2000) {
