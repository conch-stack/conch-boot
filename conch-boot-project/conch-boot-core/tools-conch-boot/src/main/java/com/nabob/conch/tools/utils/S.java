...
import com.nabob.conch.tools.constant.SysConfig;
...
public class S extends StringUtils{
	 ...
	 * @return
	 */
	public static String clearNonSysConfigt(String inputJsonStr) {
	...
	public static String toSnakeCase(String camelCaseStr){
		...
		for (char c : chars){
			if(Character.isUpperCase(c)){
				if(sb.length() > 0){
					sb.append(SysConfig.SEPARATOR_UNDERSCORE);
				}
			}
	...
	public static String toLowerCaseCamel(String snakeCaseStr){
		...
		// 不包含_
		if(!snakeCaseStr.contains(SysConfig.SEPARATOR_UNDERSCORE)){
			// 全大写直接return小写
		...
		String[] words = snakeCaseStr.split(SysConfig.SEPARATOR_UNDERSCORE);
		...
		if (snakeCaseStr.endsWith(SysConfig.SEPARATOR_UNDERSCORE) && sb != null) {
			sb.append(SysConfig.SEPARATOR_UNDERSCORE);
		}
	...
	public static String extractToken(String authToken){
		if(S.startsWithIgnoreCase(authToken, SysConfig.TOKEN_PREFIX_BEARER)){
			authToken = S.substring(authToken, SysConfig.TOKEN_PREFIX_BEARER.length()).trim();
		}
		return authToken;
	}
